...
from echosyn.common.mrrate import (VolumeTooShort, build_text_encoder, encode_boundary,
                                   encode_conditioning, encode_volume, list_series,
...
def main():
    ...
    for entry in tqdm(series, disable=None):
        ...
        latent_path = os.path.join("latents", sid[:2], f"{sid}.pt")
        embed_path = os.path.join("embeddings", sid[:2], f"{sid}.pt")

        latent_abs = os.path.join(root, latent_path)
        embed_abs = os.path.join(root, embed_path)
        try:
            ...
            if not args.overwrite and os.path.exists(latent_abs) and os.path.exists(embed_abs):
                latent = torch.load(latent_abs, map_location="cpu")
            else:
                ...
                save(latent, latent_abs)
                save(encode_conditioning(
...
def save(tensor, path):
    ...
    os.replace(tmp, path)  # atomic, so a killed array task never leaves a half-written .pt
