...
from echosyn.common.mrrate import report_sections
...
def variant_text(variant, report, modality, plane):
    ...
    if variant == "condition":
        # On this branch the generator does not condition on one string: `Conditioner.encode`
        # encodes `report_sections` -- findings, impression, acquisition -- separately and
        # concatenates the pooled vectors. HLIP has one text tower, so the same three sections are
        # flattened here in the same order. Same content the generator saw, one string instead of
        # three, which `signature()["text_source"]` records. It is therefore **not** comparable
        # with main's `hlip_condition_*`; `findings` and `impression` are built from the report
        # alone and are comparable across both branches.
        sections = [t for t in report_sections(report, modality, plane) if t.strip()]
        return "\n".join(sections) if any(report.get(k, "").strip()
                                          for k in ("findings", "impression")) else None
...
def signature():
    return {"repo": HLIP_REPO, "revision": HLIP_REVISION, "model": HLIP_MODEL,
            ...
            "text_source": "generator_conditioning: report_sections flattened "
                           "(findings + impression + acquisition)",
            "text_template": None}
