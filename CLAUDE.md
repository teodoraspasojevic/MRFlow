...
`evaluation/` scores a checkpoint with FID, FVD and Inception Score. It needs
...
**Evaluation runs two metric families over the same rollouts, on deliberately different arrays.** [`evaluation/paper_metrics.py`](evaluation/paper_metrics.py) holds FID, FVD and Inception Score, each computed by its field's reference protocol unmodified — these are the numbers the work is read by. [`evaluation/challenge_metrics.py`](evaluation/challenge_metrics.py) is the VLM3D container vendored, and still supplies MSE/PSNR/SSIM and the 2.5D FID: the challenge is over so they are nobody's reference, but they are cheap and worth logging. [`evaluation/__init__.py`](evaluation/__init__.py) holds `EvalAccumulator`, which hands each family the geometry it is defined on; [`evaluation/main.py`](evaluation/main.py) generates the pairs. **The container's habits — a squeezenet backbone, `stride=4` slice sampling, a per-slice window, a `zoom` onto the reference — may never reach `paper_metrics.py`.** The rules that make a number quotable:

- **Where a reference implementation exists, it is used rather than reimplemented.** `FID` is [`evaluation/fid_inception.py`](evaluation/fid_inception.py) — pytorch-fid v0.3.0 verbatim, so the backbone is the TF-ported `pt_inception-2015-12-05` and **not** torchvision's ImageNet Inception (different weights, no conversion factor), 2048-d pool3, images as `[0, 1]` that the module resizes and rescales itself, **every** slice, `eps*I` only on a non-finite `sqrtm`. `FVD` is StyleGAN-V's protocol: the Kinetics-400 I3D torchscript at its 400-d pre-softmax layer, called `rescale=True, resize=True, return_features=True` so the detector does its own `[0,255]→[-1,1]` and its own resize to 224², and **one clip per volume, no overlap**, short volumes discarded. `IS` is torch-fidelity's arithmetic on that same TF Inception's 1008-way logits — seed-2020 shuffle, 10 contiguous splits at `i*N//splits`, KL in float64. The shuffle makes each split a random sample of the whole set rather than a contiguous block of shards; it does **not** make the value order-invariant, so `IS_std` is still comparable only across runs with the same shard count (measured 0.069 at one shard against 0.029 at 32, while `IS_mean` moved 0.25%). Both references consume uint8, so both volumes are percentile-normalized and quantized before any feature extractor sees them. **`fid_inception.py` may not be edited**; its one marked edit is a `_sqrtm` shim for the `disp=` kwarg scipy ≥ 1.17 removed.
- **The two families never share arrays.** `compute_basic_metrics` and `FIDAccumulator` are handed `real` and `produced` exactly as they arrived, because the container normalizes both volumes its own way and `zoom`s the generated one onto the reference's shape; the reference-protocol metrics are handed the canonicalized pair. `PAPER_KEYS`/`CHALLENGE_KEYS` record which family a key belongs to, which is what says the arrays it was computed on, and `test_the_challenge_family_scores_the_released_pair_not_the_canonicalized_one` is the guard. `dice` and `FID_T` are gone — both were aliases (`SSIM_mean` and `FID_2p5D_YZ` under second names), and with a reference-protocol slice-axis `FID` present a third name for the latter is only confusing.
- **What is ours is declared, not blended in.** The geometry (both volumes canonicalized into the model's 1 mm isotropic / 256² training grid rather than the reference being left native), the slice axis (array axis 0 only — the acquisition plane, the axis the model rolls out along, so the images scored are the images generated), the 0.5/99.5 intensity window, `FVD_f64` (four blocks, reading coherence across block boundaries), the `thin`/`thick` strata, and the centered rather than random clip offset. `FVD_f16` is the only number computed by an unmodified reference protocol, which is why it leads `HEADLINE_KEYS`. These numbers are not comparable to a paper that scored native-geometry references.
...
- **`METRIC_KEYS` is also the reporting order** — `HEADLINE_KEYS` first (`FVD_f16`, `FID`, `IS_mean`, `FVD_f64`, then `FID_2p5D_Avg`, `PSNR_mean`, `SSIM_mean`, `MSE_mean`), then the remainder of `PAPER_KEYS`, the remainder of `CHALLENGE_KEYS`, then the file counts — and it drives the W&B table, the summary and the console dump at once. **Both sample counts are reported for every distance** (`n_fid_slices_real`/`_fake`, `n_fvd_f16_clips_real`/`_fake`): nothing forces a rollout to the reference's length, so equal counts are a property of the run, not of the code.
...
- **Every rollout is cached to `<out>/generated/` as fp16 `.npy`** unless `--no-cache_generated` is passed, with an `index-NNNN.json` per shard carrying modality/plane/spacing/archive/member. Generation is the expensive half (20 blocks x 201 Euler steps per case) and the metrics are minutes, so a metric change should be a rescore, not another GPU-week. ~21 MB per case, ~21 GB and 1,000 inodes at 1,000 cases — inside the 102,400 inode limit, but check it before caching a full split.
