...
- **`METRIC_KEYS` is also the reporting order** (FID average, FVD, PSNR, SSIM, MSE, per-plane FIDs, IS, then `dice` and the counts), and it drives the W&B table, the summary and the console dump at once. `dice` is a literal copy of `SSIM_mean` — the platform's primary-metric shim, not real Dice.
- **`FVD` and `IS_mean`/`IS_std` are ours, and the MR challenge scores neither**, so they live in `challenge.py`'s last section, behind a header saying so; everything above that line is byte-identical to the container, and `tests/test_evaluation_metrics.py::test_official_basic_metrics_still_score_what_they_did` keeps it that way. FVD follows the challenge's *CT* container (`ct_challenges/ctgen_evaluation`) — its 201x224x224 trilinear resize, its `[-1, 1]` video, its `FVD/fvd_pytorch_model.py` arithmetic verbatim — with two forced substitutions and one chosen one: CT-Net windows Hounsfield units and MR has none, so features come from the I3D Kinetics-400 torchscript FVD is defined over; the HU clip becomes `_normalize01`; and the container's mean-over-strata-of-4 aggregation becomes one distance pooled over every case, because a 400x400 covariance from 4 samples estimates nothing. IS is the canonical `exp(E KL(p(y|x) || p(y)))` over Inception-v3, split-averaged as the reference port does, over the generated volume's axis-0 slices only (no-reference — the ground truth never enters). Never quote either as a challenge metric.
...
- **Do not "improve" `evaluation/challenge.py`** above `### FVD and Inception Score -- NOT part of any official MR container ###`. Everything before it is the leaderboard's own arithmetic, quirks included: `stride=4` slice sampling, squeezenet1_1 rather than InceptionV3, a 0.5/99.5-percentile window recomputed per slice, and a missing case dropped from the MSE/PSNR/SSIM means instead of penalized. The only sanctioned additions to that half are marked in the file — `raw_features`/`finalize_pooled` for cross-shard FID pooling, and `_matrix_sqrt`, which drops the `disp=` kwarg scipy ≥ 1.17 removed (this venv is on 1.18, so the official file cannot run here unpatched). Cross-shard pooling agrees with a single process to ~2e-7 relative, not bit-exactly: `np.cov` over reordered float32 rows accumulates differently. The FVD/IS section below the header is ours and may be changed freely, as long as it stays below it.
- **FVD needs more than 400 cases to mean anything**, and unlike FID that is per *case*, not per slice: one 400-d I3D vector per volume, so at `--n_per_bucket 100` (1,000 scored cases) the covariance is only 2.5x the dimension — usable, biased, and not comparable across case counts. Below ~400 `sqrtm` warns that the matrix is singular and the number stops ordering: measured on single-scale synthetic volumes, noise at sigma 0.05/0.2/0.8 scored 3159/2841/840, i.e. backwards, and identically so at N=8 and N=96. On real MR the same ladder is monotone (116/543/1254/1999 for sigma 0.02/0.05/0.15/0.40, against 0.0 for an identical pair and 2331 for uniform noise), which is why the ordering tests use a broadband phantom rather than a blurred-noise one. IS on brain MRI is compressed for a different reason — ImageNet's 1,000 classes do not describe an MR slice — so it sits near 1.6-1.8, never a natural-image number.
- A shard file written before FVD/IS existed carries no `fvd_raw`/`is_probs`, so `--combine` raises on it rather than pooling a partial FVD. Re-run the array. The I3D torchscript is a first-use download into the torch hub cache (`MRFLOW_I3D_PATH` overrides it for a node with no outbound route).
